...
## mol-idea-to-plan (Demo 13)

When running `mol-idea-to-plan` from `poc-city/formulas/`:

- Default review workers: `poc-project/polecat-review` (lean agent in `agents/polecat-review/`)
- Dispatch all review legs in one batch, then `gc mail inbox` — **never** poll/sleep loops
- Read `.plan-reviews/$REVIEW_ID/state.env` after compaction to resume (`CURRENT_STEP=`)
- Do not re-read `design-doc.md` if already in context and unchanged
- Formula is optimized: 9 review legs, 10 steps (no plan-review-3)
